...
// ไอคอน LINE แบบ inline SVG (path จาก Font Awesome Free, CC BY 4.0) แทนการโหลดฟอนต์ fa-brands
// ทั้งไฟล์ (~115KB) ทั้งที่ใช้ไอคอนนี้ไอคอนเดียวในทั้งหน้า ประหยัด bandwidth ต่อการโหลดหน้านี้หนึ่งครั้ง
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    load();
    // อัปเดตทันทีอยู่แล้วผ่าน subscribeReadingsChanged (realtime) — ตั้ง interval นี้ไว้เป็นแค่ fallback
    // เผื่อ realtime หลุดชั่วคราว ไม่ต้อง poll ถี่ทุก 30 วิเหมือนเดิม ประหยัด query/แบนด์วิดท์ตอนเปิดหน้าทิ้งไว้
            ...
            <div className="glass-panel w-full max-w-md space-y-4 rounded-2xl p-6">
              <div className="flex items-center justify-between border-b border-slate-200 pb-3 dark:border-slate-800">
                <h3 className="flex items-center gap-2 font-bold text-emerald-600 dark:text-emerald-400">
                  <LineIcon className="h-6 w-6" /> การแจ้งเตือนผ่าน LINE
                </h3>
                <button
